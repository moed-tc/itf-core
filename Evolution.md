...
S = ∫ d⁴x √-g [ R/(16πG) + (1/2) g^{μν} ∂_μ τ^a ∂_ν τ_a + (1/2) g^{μν} Tr(∂_μ C ∂_ν C) + (λ/4)(Tr(C²)-3)² + (1/2) g^{μν} ∂_μ φ ∂_ν φ + (β/2) φ² (τ^a τ_a - 3)² + α R τ^a C_a{}^b τ_b ]
- Crucial coupling term: α R (τ C τ) — links external curvature, temporal misalignment, and internal compression.
- Interpretations: gravity emerges from temporal misalignment; singularities forbidden via φ divergence; dark energy from residual misalignment; dark matter from stable C modes; causality as a natural dynamic tendency.
